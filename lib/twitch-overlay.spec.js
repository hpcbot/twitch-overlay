...
describe('Server', function() {
	...
	describe('remove() - Unload Overlays', function() {
		it('Overlay does not exist: Does nothing', function() {
			overlays = new Overlays({events: eventbus})
			...
			overlays.remove(fakeOverlay)
			let updatedList = overlays.list()
			assert.equal(list.length, 1)
		})
		it('Unloads a single overlay', function() {
			overlays = new Overlays({events: eventbus})
	...
	describe('hide() - Hides an overlay by name', function() {
		before(function() {
			overlays.clearState()
		})
		it('No name: doesn\t hide an overlay', function() {
			const overlay = {
				name: 'cupadd',
				type: 'video',
				file: 'overlays/events/hpcwins.mp4'
			...
			overlays.show(list[0], null)

			overlays.hide(null)

			let state = overlays.getState()

			assert.equal(state.length, 1)
			assert.equal(state[0].name, 'cupadd')
		})
		it('Name but no overlays: does nothing', function() {
			overlays.hide('cupadd')

			let state = overlays.getState()

			assert.equal(state.length, 0)
		})
		it('Name but no match: does nothing', function() {
			const overlay = {
				name: 'cupadd',
				type: 'video',
				file: 'overlays/events/hpcwins.mp4'
		...
		it('Name matches: removes overlay', function() {
			const overlay = {
				name: 'cupadd',
				type: 'video',
				file: 'overlays/events/hpcwins.mp4'
