...
module.exports = class TwitchOverlay {
  constructor(_options) {
    ...
    this.options.events.on('overlays:remove', (overlays) => {
  ...
  remove(_overlays) {
    // Remove overlay(s) from the system
    // Expects an Object or array of Objects w/ the following structure:
    //    name: 'powermove',
    ...
    queue.forEach((item) => {
      if(item.name) {
        // Search overlays to see if item exists
        for(let i = 0; i < overlays.length; i++) {
          if(overlays[i].name == item.name) {
            // Remove item from the list of overlays
            overlays.splice(i, 1)

            // Remove dangling event listeners to prevent memory leaks or unintended calls
            this.options.events.removeAllListeners('overlay:' + item.name + ':show')
            this.options.events.removeAllListeners('overlay:' + item.name + ':hide')
          }
        }
      }
    })
