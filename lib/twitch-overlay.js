...
const app = express()    // Express should be accessible from all functions
...
let start = function start(_options) {
...
const show = function(overlay, payload) {
  let _overlay = {
    id: shortid.generate(),
    name: overlay.name,
    type: overlay.type,
  ...
  if(overlay.layout) {
    _overlay.layout = overlay.layout
  } else {
    _overlay.layout = 'center' // default layout
  }
  ...
  if(overlay.video) {
    _overlay.video = overlay.video
  }

  if(overlay.volume) {
    _overlay.volume = overlay.volume
  }

  if(overlay.audio) {
    _overlay.audio = overlay.audio
  }
...
const end = function(id, name, payload) {
