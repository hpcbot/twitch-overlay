...
var add = function(_overlays) {
  ...
  queue.forEach(function(overlay) {
    var template;
    ...
    switch(overlay.type) {
      ...
      case 'video':
        ...
        var video = new types.video(overlay.name, overlay.file);
        overlays.push(video); // Add overlay (template and payload) to our list
    ...
    addRoute(template);  // Load template and static directory so it has a URL endpoint on the server
  });
}
...
var load = function(overlay, _socket) {
    ...
    _socket.emit('load:template', overlay);

    var overlayEvent = function overlayEvent (payload) {
      io.emit('show:overlay', overlay, payload);
    }
    clients[_socket.id][overlay.event] = overlayEvent; // Keep track of overlay events so we can clear them on disconnect

    // Add listener for overlay event
    options.events.on(overlay.event, overlayEvent);
};
